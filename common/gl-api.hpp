...
inline void check_extensions(std::initializer_list<std::string> required_extensions)
{
    std::vector<bool> supported(required_extensions.size(), false);
    ...
    for (int i = 0; i<num_extensions; ++i)
    {
        ...
        {
            if (ext == required_extensions.begin()[j])
            {
                supported[j] = true;
            }
        }
    ...
    ss << "Required extensions not supported:";
    bool fail = false;
    for (size_t j = 0; j<supported.size(); ++j)
    {
        if (!supported[j])
        {
            ss << ' ' << required_extensions.begin()[j];
            fail = true;
        }
    }
    if (fail)
